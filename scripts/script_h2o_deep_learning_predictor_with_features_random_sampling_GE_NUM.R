...
require(stringr)
require(h2o)
localH2O = h2o.init(ip = "localhost", port = 54321, startH2O = TRUE, nthreads=5, max_mem_size="11G")
...
for (drug in drugs){
  ...
  temp.rules2<-temp.rules[temp.rules$support>=quantile(temp.rules$support)[2],]#25% Quantile support
  ...
  for (bagging in 1:no.of.rounds){
    ...
    model2 <-
      h2o.deeplearning(x = 2:ncol(temp.train),  # column numbers for predictors
                      ...
                      hidden = c(100,100,100), # three layers of 100 nodes
                      epochs = 200,
                      nfolds = 3
      ) 
  ...
  print("CHECK")
  filename<-paste("~/Projects/MBA/data/Prediction_h2o_dl_quantile_support_filter_GE_NUM/plots/",drug,".jpg",sep="")
...
write.table(result,file="~/Projects/MBA/data/Prediction_h2o_dl_quantile_support_filter_GE_NUM/Predictors_Report.tsv",sep='\t',row.names = F)
